...
export async function formatFlightsAsMarkdown(
  ...
  if (directFlights.length > 0) {
    ...
    directFlights.forEach((f) => {
      const formatted = formatFlightObject(f, currency);
      lines.push(`| ${formatted.airline} | ${formatted.departure} | ${formatted.arrival} | ${formatted.duration} | ${formatted.stops} | ${formatted.price} | [Book](${formatted.book}) |`);
    });
    lines.push(``);
  }
