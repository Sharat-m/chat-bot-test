...
function mcpResponse(id: string | number | null, result: unknown) {
  // If result already has _meta, use it directly
...
async function runSearch({
  ...
  if (searchCache?.key === cacheKey) {
    return mcpResponse(id, {
      flights: searchCache.structuredData.flights,
      metadata: searchCache.structuredData.searchMetadata,
      _meta: {
        ui: {
          resourceUri: widgetUrl,
        },
  ...
  const flightsWithPrices = flights.map(f => {
    // You'll need to import formatPrice or calculate it here
    const price = f.priceRaw ? `$${Math.round(f.priceRaw/1000)}` : 'Price on request';
    return {
      ...f,
      price, // Add formatted price
    };
  });
  ...
  const structuredData = {
    ...
    searchMetadata: {
      ...
      cabinClass: cabinClass.replace('CABIN_CLASS_', ''),
  ...
  searchCache = { 
    key: cacheKey, 
    result, 
    markdown: '', // We don't need markdown anymore
    structuredData 
  };

  // Return in the format ChatGPT expects for widgets
  return mcpResponse(id, {
    flights: structuredData.flights,
    metadata: structuredData.searchMetadata,
    _meta: {
      ui: {
        resourceUri: widgetUrl,
      },
